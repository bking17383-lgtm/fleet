...
def _post_say(text: str) -> None:
    ...
    full = text.strip()
    aloud = for_aloud(full) or full
    SAY_FILE.write_text(aloud + "\n", encoding="utf-8")
    _say_mtime = SAY_FILE.stat().st_mtime
    _mark_captn_speaking(aloud)
    _broadcast({"type": "say", "text": aloud, "full": full if full != aloud else None})
    _dev_log("auto_say", aloud[:100])
...
def api_reply():
    ...
    aloud = for_aloud(text) or text
    SAY_FILE.write_text(aloud + "\n", encoding="utf-8")
    _broadcast({"type": "say", "text": aloud})
    _dev_log("reply", aloud)
    return jsonify({"ok": True, "text": aloud, "full": text if text != aloud else None})
