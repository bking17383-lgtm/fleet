...
BUS = bus_root()
GEM_GRAB_HOLD = BUS / "fleet/bus/GEM_GRAB_HOLD.txt"


def _gem_grab_hold() -> bool:
    """Brian/Daddy grabbed Gem — do not stomp cpt_to_gem with LOOP PING."""
    if not GEM_GRAB_HOLD.is_file():
        return False
    age = _mtime_age(GEM_GRAB_HOLD)
    if age is None or age > 3600:
        return False
    text = GEM_GRAB_HOLD.read_text(encoding="utf-8", errors="replace")
    return "hold=YES" in text or "GEM GRAB" in text
...
def _buddy_answered(buddy_line: str, gem_text: str) -> bool:
    ...
    if "buddy ok" in gem_low and ("stay awake" in gem_low or ask[:24] in gem_low):
        return True
    ...
    if understand.is_file():
        u = understand.read_text(encoding="utf-8", errors="replace").lower()
        if "buddy ok" in u or "gem: done" in u:
            if "stay awake" in ask or "hibernat" in ask or "screen shut" in ask:
                if "stay awake" in u or "insomniac" in u or "cb1_stay_awake" in u:
                    return True
...
def forward_buddy_to_gem() -> str:
    ...
    if _buddy_answered(buddy, gem_text):
        if BUDDY_FORWARDED.is_file() and "answered=NO" in BUDDY_FORWARDED.read_text(encoding="utf-8", errors="replace"):
            BUDDY_FORWARDED.write_text(
                BUDDY_FORWARDED.read_text(encoding="utf-8", errors="replace").replace("answered=NO", "answered=YES"),
                encoding="utf-8",
            )
    ...
    if TO_GEM.is_file():
        ...
        # Do not clobber a newer Daddy job (sync law · Brian tired of 746 errors)
        gem_mtime = _mtime_age(TO_GEM)
        if gem_mtime is not None and gem_mtime < 7200:
            if "from: Daddy" in prev and ask not in prev:
                if "Word: sync" not in buddy.lower() and "word: sync" not in buddy.lower():
                    return f"skip forward — cpt_to_gem newer Daddy job ({int(gem_mtime)}s old)"
    order = f"""--- Daddy → Gem (BRIAN JOB · NOT loop) ---
time: {now}
from: Daddy on {host}

STOP posting "GEM ok — CPT loop". That is not work. Brian sees nothing.

Brian asked:
  {ask}

PASTE CARD (Drive root): insomniac.txt
READ: fleet/GEM_POST_RULE.txt

WRITE (required or job FAILED):
  fleet/bus/GEM_UNDERSTAND.txt — power fix · stay awake while plugged
  fleet/bus/gem_to_cpt.txt:
    BUDDY ok — <what you did> — {now} · host: cb1

Word: BUDDY · NO LOOP SPAM
"""
...
def reconnect_gem() -> str:
    ...
    elif st["linked"] and not _buddy_pending():
...
Loop LIVE. Hold for job. Do NOT post loop-only keepalive.
...
def ping(force_boot: bool = False) -> str:
    ...
    if _gem_grab_hold() and not force_boot:
        ping_body = f"""GEM_LOOP_PING — {now}
epoch={epoch}
cpt_host={host}
loop=GRAB HOLD · do not overwrite cpt_to_gem
read: fleet/bus/cpt_to_gem.txt · drop_pile/to_gemini/GEM_GRAB_NOW.txt
"""
        PING.write_text(ping_body, encoding="utf-8")
        return f"GEM GRAB HOLD — left cpt_to_gem alone · {PING.name}"
