...
PRESETS: dict[str, list[str]] = {
    ...
    "gl_jailbreak": [
        "Read fleet/FIRST_PRIORITY.txt · fleet/bus/GL_CONTROL_FIX.txt · drop_pile/from_cursor/JAILBREAK_ANALYSIS_FOR_CPT.md",
        "WITH Gem on CB1 Chrome: export lester/free_lester_instructions.md (real body · not AWAITING_EXPORT)",
        "Build lester/scan_cheats.json from lester/gemini_live_sorting_instructions.md (plain json on Drive)",
        "Verify fleet/bus/GL_CONTROL_FIX.txt — update if gaps",
        "Tell Gem: Puppy is #1 for :8765/:8766 deploy · you own file lane",
        "bash ~/.stan/uncle_exec.sh",
        "Post uncle_to_cpt: UNCLE CHECKIN — cb1 — <ip> — <time> · gl=<done|wait>",
    ],
    "fix_puppy": [
        "Daddy order · Uncle Linux execute on CB1 · not penguin",
        "bash ~/.stan/uncle_exec.sh — reboot agent + puppy scan (built in)",
        "Hold PUPPY PROXY until puppy64 posts real checkin · do not fake puppy_outbox",
        "LAN probe puppy64 192.168.1.4 — ping · curl :8002 / :8765 / :8766",
        "Write fleet/bus/puppy_needs.txt + cpt_to_puppy.txt if keyboard needed",
        "Post uncle_to_cpt: UNCLE CHECKIN — cb1 — <ip> — <time> · puppy=<proxy|blocked|needs_keyboard>",
    ],
    "puppy_proxy": [
        "Brian: Uncle holds Puppy NET proxy until wipe done · Firefox on checkin OK",
        "bash ~/.stan/uncle_exec.sh",
        "Post uncle_to_cpt: UNCLE CHECKIN — cb1 — <ip> — <time> · puppy=proxy",
    ],
    "wake_puppy": [
        "WAKE — keyboard on puppy64 once · Drive must mount first",
        "bash ~/GoogleDrive/MyDrive/lester/NEW_PUPPY_BOOT.sh",
        "Posts real PUPPY CHECKIN · Daddy drops TEMP on penguin same tick",
        "Then read fleet/bus/cpt_to_puppy.txt · run ONE job · post puppy_outbox",
        "Do NOT run PUPPY_ONE_COMMAND until Daddy orders mesh/sarah",
    ],
}
