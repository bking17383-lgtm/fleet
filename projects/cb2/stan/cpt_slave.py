...
def _aws_inbox_vital(bus: Path) -> str | None:
    p = bus / "fleet/bus/CPT_AWS_INBOX.txt"
    if not p.is_file():
        return None
    try:
        text = p.read_text(encoding="utf-8", errors="replace")
        if "board_changed=YES" in text:
            for ln in text.splitlines():
                if ln.strip().startswith("•"):
                    return f"AWS board: {ln.strip()[1:].strip()[:85]}"
    except OSError:
        pass
    return None


def _aws_vital(bus: Path) -> str | None:
    p = bus / "fleet/bus/AWS_VITALS.txt"
    if not p.is_file():
        return None
    try:
        text = p.read_text(encoding="utf-8", errors="replace")
        for ln in text.splitlines():
            if ln.startswith("last_brian="):
                return f"AWS terminal: {ln.replace('last_brian=', '')[:85]}"
    except OSError:
        pass
    return None


def _broadcast_vital(bus: Path) -> str | None:
    try:
        import cpt_note_grab as ng

        ng.grab()
        bc = bus / "fleet/bus/BRIAN_BROADCAST.txt"
        if not bc.is_file():
            return None
        lines = bc.read_text(encoding="utf-8", errors="replace").splitlines()
        for i, ln in enumerate(lines):
            if ln.startswith("  ") and len(ln.strip()) > 8:
                return f"Broadcast: {ln.strip()[:90]}"
            if ln.startswith("[mesh]") and i + 1 < len(lines) and lines[i + 1].startswith("  "):
                return f"Broadcast: {lines[i + 1].strip()[:90]}"
    except Exception:
        pass
    return None
...
def run() -> str:
    ...
    if bc := _broadcast_vital(bus):
        vital.insert(0, bc)
    elif ai := _aws_inbox_vital(bus):
        vital.insert(0, ai)
    elif av := _aws_vital(bus):
        vital.insert(0, av)
