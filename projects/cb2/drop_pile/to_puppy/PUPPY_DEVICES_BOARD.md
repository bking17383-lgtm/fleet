...
| **Phones (fleet)** | Read `fleet/PHONE_FLEET_IDS.txt` — IDs are fleet-wide, not on puppy64 |

Use **RUNNING / DOWN / UNKNOWN**. Stale outbox ≠ working.

Phones for Puppy proposal (Brian common devices — post in PUPPY_DEVICES PHONES section):

| fleet_id | device | network | callsign |
|----------|--------|---------|----------|
| phone-moto-lte | Moto Play | cellular | ROVER |
| phone-samsung-wifi | Samsung | wifi/dongle only | TESTER |
...
PHONES (fleet registry — not hosted on puppy64)
  phone-moto-lte      ROVER   cellular · ack: lester6_to_rover.md
  phone-samsung-wifi  TESTER  wifi-only · ack: lester6_to_tester.md
  canonical: fleet/PHONE_FLEET_IDS.txt
