...
def slave_ok(ack: Path, hb: Path, callsign: str) -> bool:
    ...
    if not t or "mode: slave" not in t.lower() or f"callsign: {callsign.lower()}" not in t.lower():
        return False
    if _ack_interim(t) or not h or _hb_down(h) or not _hb_fresh(h):
        return False
...
def main() -> None:
    ...
    compact = f"CB1{sym.get(cb1,'?')} CB2{sym.get(cb2,'?')} puppy64{sym.get(pup,'?')} ROVER✓"
    working = sum(1 for r in rows if r[2] == "WORKING") + 1  # + ROVER
...
COMPUTERS (4)
  ...
  ROVER    ✓ WORKING  cell / Spark (legs WAIT)
