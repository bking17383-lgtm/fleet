...
python3 "${STAN}/gem_agent.py" once || true
python3 "${STAN}/aws_keys_pull.py" 2>/dev/null || true
python3 "${STAN}/cpt_slave.py"
bash "${STAN}/push_fleet_to_drive.sh" 2>/dev/null || true
if python3 -c "from bus_lane import is_drive_live; exit(0 if is_drive_live() else 1)" 2>/dev/null; then
  [[ -x "${STAN}/team_backup.sh" ]] && timeout 30 bash "${STAN}/team_backup.sh" failsafe_$(date +%Y%m%d) 2>/dev/null || true
fi

if ! pgrep -f "drive_watch.sh" >/dev/null 2>&1; then
  nohup bash "${STAN}/drive_watch.sh" >> "${STAN}/logs/drive_watch.log" 2>&1 &
fi
if ! pgrep -f "gem_agent.py watch" >/dev/null 2>&1; then
  nohup python3 "${STAN}/gem_agent.py" watch >> "${STAN}/logs/gem_agent.log" 2>&1 &
fi
