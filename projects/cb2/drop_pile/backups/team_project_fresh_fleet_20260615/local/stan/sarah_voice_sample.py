...
LOG = DRIVE / "fleet" / "bus" / "SARAH_VOICE_LOG.txt"
DEV_LOG = DRIVE / "fleet" / "bus" / "SARAH_DEV_LOG.txt"
DEV_RING: list[str] = []
DEV_RING_MAX = 200
...
def _guess_lan_ip() -> str | None:
    ...
    for line in subprocess.check_output(["ip", "-4", "addr"], text=True).splitlines():
        ...
        if m:
            return m.group(1)
    if PHONE_URL_FILE.is_file():
        t = PHONE_URL_FILE.read_text(encoding="utf-8").strip()
...
def _phone_base_url() -> str:
    if PHONE_URL_FILE.is_file():
        line = PHONE_URL_FILE.read_text(encoding="utf-8").strip().splitlines()[0]
...
def kid_face():
    path = KID_FACE if KID_FACE.is_file() else DRIVE / "convert_inbox" / "20260615_052625.jpg"
    if path.is_file():
...
def api_devlog():
    if DEV_LOG.is_file():
        lines = DEV_LOG.read_text(encoding="utf-8", errors="replace").splitlines()[-120:]
    else:
...
def api_voice_url():
    if HTTPS_URL_FILE.is_file():
        url = HTTPS_URL_FILE.read_text(encoding="utf-8").strip().splitlines()[0]
        if url.startswith("http"):
