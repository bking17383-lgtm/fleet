...
try:
    from camel_art import show_slot
except ImportError:
    def show_slot(slot, fallback=""):
        print(fallback)
...
GAME_VERSION = "v0.8.0"
SILK_ROAD_DAYS = 20
SILK_ROAD_ENTRY_FEE = 1200
SILK_ROAD_MIN_BAC = 1
SILK_BALE_VALUE = (180, 320)

SILK_ROAD_NODES = [
    {"name": "Pollnivneach Outpost", "region": "Northeast fringe", "trade": True},
    {"name": "Shanty Pass", "region": "Rocky saddle", "trade": False},
    {"name": "Bedouin Relay", "region": "Smoke on the horizon", "trade": True},
    {"name": "Oasis of Broken Wells", "region": "Bitter water", "trade": False},
    {"name": "Salt Flats Crossing", "region": "White blindness", "trade": False},
    {"name": "Petra Caravanserai", "region": "Rose-red cliffs", "trade": True},
    {"name": "Wadi Rum Narrows", "region": "Wind-scoured stone", "trade": False},
    {"name": "Aqaba Smoke", "region": "Salt air", "trade": True},
    {"name": "Sinai Footpath", "region": "Holy dust", "trade": False},
    {"name": "Red Sea Ridge", "region": "Heat mirage", "trade": False},
    {"name": "Nile Delta Fringe", "region": "Papyrus smell", "trade": True},
    {"name": "Memphis Bazaars", "region": "Old kingdom haggling", "trade": True},
    {"name": "Syrian Gate", "region": "Fortified toll", "trade": False},
    {"name": "Palmyra Ruins", "region": "Columns in sand", "trade": True},
    {"name": "Euphrates Ferry", "region": "River bargaining", "trade": True},
    {"name": "Baghdad Spice Row", "region": "Cinnamon air", "trade": True},
    {"name": "Persian Plateau", "region": "Blue tile distance", "trade": False},
    {"name": "Herat Bazaar", "region": "Lapis traders", "trade": True},
    {"name": "Oxus Crossing", "region": "Cold current", "trade": False},
    {"name": "Samarkand Gate", "region": "Silk and bells", "trade": True},
]
...
class CamelClicker:
    ...
    def to_dict(self):
        return {
            ...
            "silk_road_node": self.silk_road_node,
            "silk_bales": self.silk_bales,
            "silk_cargo": self.silk_cargo,
            "silk_road_log": self.silk_road_log[-12:],
    ...
    def load(self):
        if os.path.exists(SAVE_FILE):
            try:
                ...
                self.silk_road_node = data.get("silk_road_node", 0)
                self.silk_bales = data.get("silk_bales", 0)
                self.silk_cargo = data.get("silk_cargo", [])
                self.silk_road_log = data.get("silk_road_log", [])
    ...
    def capture_home_snapshot(self, force=False):
        """Picture of home + herd (+ kids) — merchant carries this memory on the road."""
        if self.on_silk_road and not force:
    ...
    def can_start_silk_road(self):
        if self.on_silk_road:
            return False, "Already on the road east."
        if self.camels.get("bac", 0) < SILK_ROAD_MIN_BAC:
            return False, "Need a Bactrian (2-hump) — the Silk Road demands two humps."
        if self.is_bedridden():
            return False, "Too ill to depart."
        if self.coins < SILK_ROAD_ENTRY_FEE:
            return False, f"Caravan papers and feed bond: ${SILK_ROAD_ENTRY_FEE:,}."
        return True, ""

    def start_silk_road(self):
        ok, reason = self.can_start_silk_road()
        if not ok:
            return reason
        self.coins -= SILK_ROAD_ENTRY_FEE
        self.capture_home_snapshot(force=True)
        self.on_silk_road = True
        self.silk_road_days_left = SILK_ROAD_DAYS
        self.silk_road_node = 0
        self.silk_bales = 0
        self.silk_cargo = []
        self.silk_road_log = ["Departure from Pollnivneach. Home frozen in portrait."]
        return (
            f"East! {SILK_ROAD_DAYS} road-days. Bactrian leads the string. "
            f"Home time stops — check portrait P."
        )

    def silk_road_at_end(self):
        return (
            self.silk_road_days_left <= 0
            or self.silk_road_node >= len(SILK_ROAD_NODES) - 1
        )

    def _silk_road_event(self, node, rested):
        roll = random.random()
        if rested and roll < 0.35:
            return "Caravan songs. Nothing lost."
        if roll < 0.18:
            loss = random.randint(120, 420)
            if self.coins >= loss:
                self.coins -= loss
                return f"Bandits! Paid ${loss:,} to pass."
            self.health -= random.randint(8, 16)
            return "Bandits! Beaten and searched — little left."
        if roll < 0.30:
            self.health -= random.randint(4, 10)
            return "Sandstorm — lips crack, humps ache."
        if roll < 0.48 and node.get("trade"):
            bales = random.randint(1, 2)
            self.silk_bales += bales
            return f"Trader deal: +{bales} silk bale(s) ({self.silk_bales} total)."
        if roll < 0.62:
            item = random.choice(FOREIGN_SHARE_ITEMS)
            self.silk_cargo.append(item)
            return f"Picked up cargo: {item}."
        if roll < 0.74:
            self.coins += random.randint(90, 280)
            return "Side gamble won — a few honest coins."
        if roll < 0.84:
            relic_id = random.choice(["mirage_water", "desert_coin", "healing_clay"])
            self.holy_relics.append(relic_id)
            return f"Roadside shrine: {RELIC_DEFS[relic_id]['name']}."
        if roll < 0.92:
            art = random.choice(ODDBALL_ARTIFACTS)
            self.silk_cargo.append(art)
            return f"Odd find packed for home: {art}."
        return node.get("region", "Dust") + " — another day east."

    def advance_silk_day(self, rest=False):
        if not self.on_silk_road:
            return "Not on the Silk Road."
        if self.silk_road_at_end() and not rest:
            return "Samarkand gate reached — press R to turn home."
        if self.silk_road_days_left <= 0:
            return "Road-days spent — press R to return home."
        self.silk_road_days_left -= 1
        if self.silk_road_node < len(SILK_ROAD_NODES) - 1:
            self.silk_road_node += 1
        node = SILK_ROAD_NODES[self.silk_road_node]
        if rest:
            gain = random.randint(8, 18)
            self.health = min(100, self.health + gain)
            msg = f"Rest at {node['name']}. +{gain} health."
        else:
            self.health -= random.randint(2, 9)
            msg = f"Marched to {node['name']} ({node['region']})."
        self.health = max(0, self.health)
        msg += " " + self._silk_road_event(node, rest)
        self.silk_road_log.append(msg)
        if self.health <= 0:
            return self._silk_road_fail()
        if self.silk_road_at_end():
            msg += " Journey complete — menu R: return home."
        return msg

    def _silk_road_fail(self):
        lost_bales = self.silk_bales
        self.silk_bales = max(0, self.silk_bales // 2)
        self.silk_cargo = self.silk_cargo[: len(self.silk_cargo) // 2]
        self.health = 22
        self.on_silk_road = False
        self.silk_road_days_left = 0
        self.capture_home_snapshot(force=True)
        return (
            f"Collapse on the road! Dragged home half-dead. "
            f"Lost {lost_bales - self.silk_bales} bale(s). Health {int(self.health)}%."
        )

    def _apply_silk_homecoming(self):
        """Home did not advance days — small random drift while you were east."""
        roll = random.random()
        if self.wife == "title":
            tribute = min(self.dung, WIVES["title"]["daily_dung"] * 3)
            self.dung = max(0, self.dung - tribute)
            self.pending_events.append(
                f"While you were east: Fatima kept order (-{int(tribute)} dung tribute)."
            )
        elif self.wife == "kind":
            self.health = min(100, self.health + 6)
            self.pending_events.append("Layla left herbs by the stable door.")
        elif self.wife == "kids" and self._living_kids():
            kid = random.choice(self._living_kids())
            self.pending_events.append(
                f"{kid['name']} drew maps of Samarkand in the dust — wrong continent."
            )
        elif roll < 0.35:
            herd = self.get_idle_rate()
            if herd:
                self.dung += herd
                self.pending_events.append(
                    f"Home herd pooped without you (+{herd} dung — days frozen, dung isn't)."
                )
        elif roll < 0.55:
            self.market_price = min(6, self.market_price + 1)
            self.pending_events.append("Market ticked up while caravans gossiped.")
        else:
            self.pending_events.append("Stable stood quiet. Nothing burned. Small mercy.")

    def return_from_silk_road(self):
        if not self.on_silk_road:
            return "You are already home."
        if not self.silk_road_at_end() and self.silk_road_days_left > 0:
            return "Too early — finish the road or march until days run out."
        payout = self.silk_bales * random.randint(*SILK_BALE_VALUE)
        self.coins += payout
        for item in self.silk_cargo:
            if random.random() < 0.55:
                self.artifacts.append(item)
            else:
                self.pending_events.append(f"Souvenir from the east: {item}")
        self._apply_silk_homecoming()
        bales = self.silk_bales
        cargo_n = len(self.silk_cargo)
        self.on_silk_road = False
        self.silk_road_days_left = 0
        self.silk_bales = 0
        self.silk_cargo = []
        ...
        self.capture_home_snapshot(force=True)
        return (
            f"Home at day {int(self.game_day)} (calendar frozen while away). "
            f"Sold {bales} silk bale(s) for ${payout:,}. "
            f"{cargo_n} cargo piece(s) unpacked."
        )
...
def silk_road_menu(game):
    msg = ""
    while game.on_silk_road:
        clear_screen()
        node = SILK_ROAD_NODES[game.silk_road_node]
        print("=============================================================")
        print(" 🐪  SILK ROAD — Bactrian expedition (home time frozen)")
        print(f" {GAME_VERSION}  |  Road-days left: {game.silk_road_days_left}/{SILK_ROAD_DAYS}")
        print("=============================================================")
        print(f" 📍 {node['name']} — {node['region']}")
        print(f" ❤️  {health_bar(game.health)}  |  🧵  Silk bales: {game.silk_bales}")
        print(f" 🪙  Coins: ${int(game.coins):,}  |  📦  Cargo: {len(game.silk_cargo)}")
        print(f" 🏠  Home portrait locked day {game.home_snapshot.get('day', '?') if game.home_snapshot else '?'}")
        print("-------------------------------------------------------------")
        if msg:
            print(f" 💬 {msg}")
            print("-------------------------------------------------------------")
            msg = ""
        if game.silk_road_log:
            print(" Recent:")
            for line in game.silk_road_log[-3:]:
                print(f"  · {line[:72]}")
        print("-------------------------------------------------------------")
        print(" 1. March east (1 road-day)")
        print(" 2. Rest at caravanserai (1 road-day, +health)")
        if node.get("trade") and game.silk_bales > 0:
            print(" 3. Sell 1 bale here (instant coin, no day)")
        print(" P. Home portrait (frozen memory)")
        if game.silk_road_at_end():
            print(" R. Return home (resolve cargo)")
        print(" Q. Save & quit")
        print("=============================================================")
        choice = input(" Road action: ").strip().lower()
        if choice == "1":
            msg = game.advance_silk_day(rest=False)
            game.save()
        elif choice == "2":
            msg = game.advance_silk_day(rest=True)
            game.save()
        elif choice == "3" and node.get("trade") and game.silk_bales > 0:
            game.silk_bales -= 1
            gain = random.randint(*SILK_BALE_VALUE)
            game.coins += gain
            msg = f"Sold 1 bale at {node['name']} for ${gain:,}."
            game.save()
        elif choice == "p":
            msg = portrait_menu(game)
        elif choice == "r":
            msg = game.return_from_silk_road()
            game.save()
            if not game.on_silk_road:
                return msg
        elif choice == "q":
            game.save()
            print("\nSaved on the road. Camels remember.\n")
            raise SystemExit(0)
        else:
            msg = "March (1), rest (2), portrait P, or return R when journey ends."
    return msg
...
def stable_menu(game):
    while True:
        ...
        for i, (k, label) in enumerate(
            ...
            if k == "gld":
                print("    Golden — ??? (mystery asset; races fast, reveal deferred)")
            if on_circuit:
...
def main():
    ...
    while True:
        ...
        print("=============================================================")
        show_slot("04_yard_day", ASCII_CAMEL)
        print("=============================================================")
        ...
        if game.camels.get("bac", 0) >= SILK_ROAD_MIN_BAC and not game.on_silk_road:
            fee = SILK_ROAD_ENTRY_FEE
            print(f" T. 🐪 Silk Road expedition (~{SILK_ROAD_DAYS} road-days, ${fee:,} papers)")
        ...
        if choice == "":
            ...
            if game.on_silk_road:
                ...
                ok, reason = game.can_start_silk_road()
                if not ok:
                    msg = reason
                elif input(f" Depart east? (${SILK_ROAD_ENTRY_FEE:,} + frozen home) (y/n): ").strip().lower() == "y":
                    msg = game.start_silk_road()
                else:
                    msg = "The Bactrian waits. Ali shrugs."
            game.save()
        elif choice == "8":
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "--selftest":
        ...
        g.camels["bac"] = 1
        g.coins = 8000
        home_day = int(g.game_day)
        msg = g.start_silk_road()
        assert g.on_silk_road, msg
        frozen = g.home_snapshot["day"]
        g.dung += 99
        for _ in range(SILK_ROAD_DAYS):
            g.advance_silk_day(rest=False)
        assert g.silk_road_at_end()
        assert g.home_snapshot["day"] == frozen
        ret = g.return_from_silk_road()
        assert not g.on_silk_road, ret
        assert int(g.game_day) == home_day
        print("selftest: silk road v0.8 ok")
        sys.exit(0)
    main()
