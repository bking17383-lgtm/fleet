...
def main() -> int:
    ...
    if role not in ROLES:
        ...
        return 1
    expect, orders_rel, _out_rel = ROLES[role]
    key = _key(role)
    if key != expect:
        print(f"BAD KEY for {role} — need {expect[:4]}… in ~/.stan/slave.key")
        return 1
    drive = bus_root()
    host = socket.gethostname()
    now = datetime.now(timezone.utc).astimezone().isoformat(timespec="seconds")
    if not (drive / "fleet").is_dir():
        print(f"NO BUS — {role} {host} {now} (Drive not mounted?)")
        return 2
    orders = drive / orders_rel
    print(f"OK {role} key={key[:4]}… host={host} bus={drive}")
    if orders.is_file():
