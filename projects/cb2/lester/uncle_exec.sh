...
PUPPY_LINE=""
if grep -qiE 'puppy|fix_puppy|proxy' "${FROM_CPT}" "${ORDERS}" 2>/dev/null; then
  PING=DOWN
  ping -c 1 -W 2 "${PUPPY_IP}" >/dev/null 2>&1 && PING=UP
  port_ok() {
    curl -sf --connect-timeout 2 "http://${PUPPY_IP}:$1/health" >/dev/null 2>&1 \
      || curl -sf --connect-timeout 2 "http://${PUPPY_IP}:$1/" >/dev/null 2>&1
  }
  P8002=DOWN; P8765=DOWN; P8766=DOWN
  port_ok 8002 && P8002=UP
  port_ok 8765 && P8765=UP
  port_ok 8766 && P8766=UP
  if [[ "${PING}" == UP ]] && [[ "${P8765}" == UP || "${P8766}" == UP ]]; then
    PUPPY_STATE="reachable"
  elif [[ "${PING}" == UP ]]; then
    PUPPY_STATE="needs_keyboard"
  else
    PUPPY_STATE="proxy"
  fi
  PUPPY_LINE="puppy=${PUPPY_STATE} lan=${PUPPY_IP} ping=${PING} :8002=${P8002} :8765=${P8765} :8766=${P8766}"
  PROXY="${BUS}/fleet/bus/UNCLE_PUPPY_PROXY.txt"
  {
    echo "UNCLE PUPPY PROXY — ${NOW}"
    echo "${PUPPY_LINE}"
    echo "Daddy TEMP on penguin until real PUPPY CHECKIN"
  } > "${PROXY}"
  NEEDS="${BUS}/fleet/bus/puppy_needs.txt"
  {
    echo "puppy_needs — ${NOW} · Uncle scan from cb1"
    echo "${PUPPY_LINE}"
    echo "keyboard: bash ~/GoogleDrive/MyDrive/lester/NEW_PUPPY_BOOT.sh"
  } > "${NEEDS}"
  log "puppy scan: ${PUPPY_LINE}"
fi

{
  cat "${TO_CPT}.tmp"
  echo "aws_keys=${AWS_STATE}"
  [[ -n "${PUPPY_LINE}" ]] && echo "${PUPPY_LINE}"
  echo "loop=TWO_WAY"
  echo "UNCLE CHECKIN — cb1 — ${IP} — ${NOW}"
  [[ -n "${PUPPY_LINE}" ]] && echo "role=UNCLE PUPPY PROXY until puppy64 live"
