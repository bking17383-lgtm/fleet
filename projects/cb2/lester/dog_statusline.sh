#!/usr/bin/env python3
"""Cursor statusLine — DOG · honest · no Composer · no Puppy lie."""
import json
import re
import sys
...
def _dog_poison(body: str) -> bool:
    low = body.lower()
    if "daddy relay" in low or "daddy temp" in low or "puppy temp" in low:
        return True
...
def _dog_ok(body: str, fresh: bool) -> bool:
    if not fresh or not body.strip() or _dog_poison(body):
        return False
...
def _gem_ok(body: str, fresh: bool) -> bool:
    if not fresh or not body.strip() or "penguin" in body.lower():
        return False
    return bool(
        re.search(r"^(GEM|BUDDY) (ok|REFUSE|—)", body, re.I | re.M)
        or re.search(r"read ping", body, re.I)
    )
...
def _box_from_checkin(text: str, box: str) -> str:
    ...
    for line in text.splitlines():
        ...
        if "CHECKED IN" in rest:
            return "✓"
        if any(x in rest for x in ("STALE", "MOVED", "PARTIAL", "LOOP", "ABSENT", "NOT GREEN", "KILLED")):
            return "~"
        return "✗"
    return "✗"
...
def main() -> None:
    ...
    print("\033[90mYou are DOG · not Composer · not Puppy · read dog.txt\033[0m")
